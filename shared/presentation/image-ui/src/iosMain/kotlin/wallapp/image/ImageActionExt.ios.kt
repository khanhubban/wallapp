    ...
    when (action) {
        is SeikoImageAction.Failure -> {
            when (val error = action.error) {
                is IllegalArgumentException -> ImageAction.Error.ImageDecodeError(imageUrl, error)
                else -> ImageAction.Error.GeneralError(imageUrl, error).also {
                    Log.e(error.message.toString(), error)
                }
            }
        }
